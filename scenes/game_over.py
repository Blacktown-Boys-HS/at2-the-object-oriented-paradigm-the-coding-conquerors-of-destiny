...
class GameOverMenu:
    ...
    def render(self, screen, time_seconds):
        ...
        draw_gothic_title(
            screen, self.title_font, "GAME OVER", SCREEN_WIDTH // 2, 220
        )

        subtitle = self.credit_font.render(
            "You fell in the dungeon.", FONT_ANTIALIAS, SUBTITLE_COLOR
        )
        subtitle_rect = subtitle.get_rect(center=(SCREEN_WIDTH // 2, 310))
        screen.blit(subtitle, subtitle_rect)

        selection_rect = pygame.Rect(
            SCREEN_WIDTH // 2 - 190, int(self.selection_y) - 6, 380, 60
        )
        draw_gothic_selection_box(
            screen,
            selection_rect,
