...
def draw_player_health_bar(screen, player, camera, zoom):
    ...
    # Background
    pygame.draw.rect(
        screen, (60, 0, 0), (bar_x, bar_y, bar_width, bar_height), border_radius=3
    )
    ...
    if fill_width > 0:
        pygame.draw.rect(
            screen, fill_color, (bar_x, bar_y, fill_width, bar_height), border_radius=3
        )

    # Border
    pygame.draw.rect(
        screen, (0, 0, 0), (bar_x, bar_y, bar_width, bar_height), 1, border_radius=3
    )
...
def draw_door_prompt(screen, player, camera, zoom, font):
    ...
    # Border
    pygame.draw.rect(
        screen, (130, 130, 130), (box_x, box_y, box_w, box_h), 2, border_radius=8
    )
...
def draw_key_prompt(screen, player, camera, zoom, font):
    """Draw a styled key interaction prompt above the player."""
    screen_x = (player.position.x - camera.x) * zoom + SCREEN_WIDTH / 2
    screen_y = (player.position.y - camera.y) * zoom + SCREEN_HEIGHT / 2

    # Render text
    arrow = font.render("> E ", FONT_ANTIALIAS, (200, 180, 80))
    label = font.render("Pick Up Key", FONT_ANTIALIAS, (255, 255, 255))
    ...
    # Border
    pygame.draw.rect(
        screen, (200, 180, 80), (box_x, box_y, box_w, box_h), 2, border_radius=8
    )
